#!/usr/bin/env python3
'''
This file generates three Maze objects representing the three physical mazes to be
solved. The walls, holes and checkpoints were all located manually and entered into
this file. Checkpoint positioning was optimised for the quickest time of solve, with
"special checkpoints" implemented to tackle especially ticky areas. See the Checkpoint
class in objects.py for more information.
'''
...
''' MAZE 1 START '''
Ball1 = Ball(np.array([FrameSide[0] + 123, FrameSide[1] + 8])) # Set ball near to first checkpoint.
...
for Wall_ in Walls1: # Add frame width.
...
for Hole_ in Holes1: # Add frame width.
    Hole_.S = Hole_.S + FrameSide

# Generate the checkpoints.
Checkpoints1 = []
# JJ wrote the checkpoints in tuple format.
Maze1Points = (
(124, 8),
(70, 8, (12, 1.5, np.array([None, None]))),
(7, 107, (8, 0, np.array([-pi/4.5, None]))),
(100, 68),
(105, 110),
(85, 146),
(57, 122, (5.5, 0.7, np.array([None, None]))),
(17, 160, (14, 0, np.array([-pi/4.9, pi/3.4]))),
(64, 225),
(68, 168),
(90, 170),
(95, 197),
(120, 197),
(118, 165),
(175, 130, (8, 0.5, np.array([pi/5, None]))),
(125, 145, (8, 0.5, np.array([-pi/5, None]))),
(124, 85, (8, 0.5, np.array([-pi/5, None]))),
(218, 71),
(163, 7),
(270, 7),
(270, 49),
(233, 67, (8, 0, np.array([-pi/6, None]))),
(269, 113, (8, 0, np.array([pi/5, None]))),
(233, 148, (8, 0, np.array([-pi/5, None]))),
(265, 180), (222, 225, (10, 0, np.array([None, None]))),
(192, 168, (8, 0, np.array([-pi/4, -pi/5]))),
(92, 222)
)
# Generate checkpoints in correct format, accounting for special settings.
for Point in Maze1Points:
    if len(Point) == 2:
        Coordinates = np.array([Point[0], Point[1]]) + FrameSide
        Checkpoint_ = Checkpoint(Coordinates)
        Checkpoints1.append(Checkpoint_)
    elif len(Point) == 3:
        Coordinates = np.array([Point[0], Point[1]]) + FrameSide
        Checkpoint_ = Checkpoint(Coordinates, True, Point[2][0], Point[2][1], Point[2][2]) # Order: radius, time, hard control signal.
        Checkpoints1.append(Checkpoint_)

# Generate Maze with all elements.
Maze1 = Maze(Ball1, Walls1, Holes1, Checkpoints1)
''' MAZE 1 END '''

''' MAZE 2 START '''
Ball2 = Ball(np.array([FrameSide[0] + 158, FrameSide[1] + 12])) # Set ball near to first checkpoint.
...
for Point in Maze2Points:
    if len(Point) == 2:
        Coordinates = np.array([Point[0], Point[1]]) + FrameSide
        Checkpoint_ = Checkpoint(Coordinates)
        Checkpoints2.append(Checkpoint_)
    elif len(Point) == 3:
        Coordinates = np.array([Point[0], Point[1]]) + FrameSide
        Checkpoint_ = Checkpoint(Coordinates, True, Point[2][0], Point[2][1], Point[2][2]) # Order: radius, time, hard control signal.
        Checkpoints2.append(Checkpoint_)
...
# Generate Maze with all elements.
Maze2 = Maze(Ball2, Walls2, Holes2, Checkpoints2)
''' MAZE 2 END '''

''' MAZE 3 START '''
Ball3 = Ball(np.array([FrameSide[0] + 142, FrameSide[1] + 17])) # Set ball near to first checkpoint.
...
for Point in Maze3Points:
    if len(Point) == 2:
        Coordinates = np.array([Point[0], Point[1]]) + FrameSide
        Checkpoint_ = Checkpoint(Coordinates)
        Checkpoints3.append(Checkpoint_)
    elif len(Point) == 3:
        Coordinates = np.array([Point[0], Point[1]]) + FrameSide
        Checkpoint_ = Checkpoint(Coordinates, True, Point[2][0], Point[2][1], Point[2][2]) # Order: radius, time, hard control signal.
        Checkpoints3.append(Checkpoint_)
...
# Generate Maze with all elements.
Maze3 = Maze(Ball3, [], [], Checkpoints3)
''' MAZE 3 END '''

if __name__ == "__main__":
    # Check that all three mazes are properly defined. 
    if type(Maze1) != Maze:
