...
def Reset():
    ...
    os.system('echo ' + str(100000 * motor_limit[1]) + ' > /sys/devices/platform/soc/fe20c000.pwm/pwm/pwmchip0/pwm0/duty_cycle') # Set up the on time, which makes the motor reset to 90 degree.
    ...
    os.system('echo ' + str(100000 * motor_limit[1]) + ' > /sys/devices/platform/soc/fe20c000.pwm/pwm/pwmchip0/pwm1/duty_cycle')
    os.system('echo 1 > /sys/devices/platform/soc/fe20c000.pwm/pwm/pwmchip0/pwm1/enable')

def change_angle(Theta):
    # Theta (radians) should be a size 2 vector of floats. i.e. np.array([0.2 * pi, 0.2 * pi])
    On_time = 100000 * Theta / motor_steps # New on times
    
    # For pin12
    os.system('echo ' + str(On_time[0]) + ' > /sys/devices/platform/soc/fe20c000.pwm/pwm/pwmchip0/pwm0/duty_cycle') # Set up the new on time for the new angle.
    # For pin35
    os.system('echo ' + str(On_time[1]) + ' > /sys/devices/platform/soc/fe20c000.pwm/pwm/pwmchip0/pwm1/duty_cycle')
